use crate::{
    models::{
        message::Message,
        proto::{
            ErrorType, Input, InputParcel, JoinedInput, JoinedOutput, Output, OutputParcel,
            PostedImageInput, PostedInput, PostedOutput, SelfJoinedOutput,
        },
        user::User,
    },
    GlobalFeed, Users,
...
impl Hub {
    ...
    pub async fn broadcast(&self, output: Output) {
        ...
        self.users.read().await.iter().for_each(move |user| {
            self.sender
                .send(OutputParcel::new(user.get_id(), output.clone()))
                .unwrap();
        });
    }
    ...
    pub async fn send_ignored(&self, output: Output, client_id: Uuid) {
        self.users
            ...
            .for_each(|usr| {
                self.sender
                    .send(OutputParcel::new(usr.get_id(), output.clone()))
                    .unwrap();
            });
    }
    ...
    pub async fn disconnect_user(&self, client_id: Uuid) {
        let result = self
            ...
            .await
            .iter()
            .position(|usr| usr.get_id() == client_id);
    ...
    pub async fn process(&self, input: InputParcel) {
        match input.input {
            ...
            Input::PostedImage(msg) => self.process_image(msg, input.client_id).await,
        }
    }
    ...
    pub async fn process_join(&self, input: JoinedInput, client_id: Uuid) {
        if self
            ...
            .await
            .iter()
            .find(|usr| usr.get_nickname() == input.nickname)
            .is_some()
        {
        ...
        self.send_directed(
            Output::SelfJoined(SelfJoinedOutput {
    ...
    pub async fn process_image(&self, input: PostedImageInput, client_id: Uuid) {
        let img = &input.content[input.content.len() - 50..input.content.len()];
        println!("deeeeeeee {:?}", img);
        ...
        let message = Message::new(user.get_id(), &response);
        self.feed.write().await.add_message(message);
...
pub mod tests {
    ...
    async fn test_broadcasting() {
        ...
        tokio::spawn(async move {
            tx.send(InputParcel {
                client_id: vanea_clone.clone().get_id(),
                input: Input::Joined(JoinedInput {
                    nickname: "vanea".to_string(),
                }),
            })
            .unwrap();
            tx.send(InputParcel {
                client_id: vanea_clone.clone().get_id(),
                input: Input::Posted(PostedInput {
                    content: "cf???".to_string(),
                }),
            })
            .unwrap();
            hub.run(rx).await;
        ...
        assert_eq!(
            ...
            OutputParcel {
                client_id: vanea.get_id(),
                output: Output::Joined(JoinedOutput {
                    nickname: "vanea".to_string()
                })
            }
        );
        assert_eq!(
            ...
            OutputParcel {
                client_id: vanea.get_id(),
                output: Output::Posted(PostedOutput {
                    content: "cf???".to_string()
                })
            }
        );
