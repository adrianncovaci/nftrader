...
use crate::{GlobalFeed, Users, models::{message::Message, proto::{ErrorType, Input, InputParcel, JoinedInput, JoinedOutput, Output, OutputParcel, PostedInput, PostedOutput}, user::User}};
...
impl Hub {
    ...
    pub async fn process_join(&self, input: JoinedInput, client_id: Uuid) {
        if self.users.read().await.iter().find(|usr| usr.get_id() == client_id).is_some() {
